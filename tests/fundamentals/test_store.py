...
class TestAppendFacts:
    ...
    def test_creates_schema_and_commits_one_atomic_batch(self):
        connection = _FakeConnection(one_rows=(("batch-001",), (1,)))

        inserted = append_facts((_fact(),), conn=connection)
    ...
    def test_idempotent_replay_does_not_insert_or_fail(self):
        fact = _fact()
    ...
    def test_conflicting_replay_rolls_back_the_whole_batch(self):
        fact = _fact()
    ...
    def test_reused_batch_id_with_different_metadata_is_rejected(self):
        fact = _fact()
        connection = _FakeConnection(
            one_rows=(
                None,
        ...
        assert ("rollback",) in connection.events
        assert INSERT_FACT_SQL not in [
            event[1] for event in connection.events if event[0] == "execute"
        ]
