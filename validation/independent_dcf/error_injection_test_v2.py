#!/usr/bin/env python3
"""
error_injection_test_v2.py -- Deliberate error-injection check for V2, per
docs/independent-validation-plan.md §Deliberate error-injection checks.

Track A Phase 2C. Same technique as V1's error_injection_test.py (preserved
unchanged), retargeted at independent_dcf_validation_v2.xlsx / shadow_calc_v2.py.

Makes a TEMPORARY copy of independent_dcf_validation_v2.xlsx, replaces the
...
else in this validation (shadow_calc_v2.py) -- applied here with the ONE
...
correct workbook (independent_dcf_validation_v2.xlsx) is retained.
"""
import json
import os
import shutil
...
ORIGINAL = os.path.join(HERE, "independent_dcf_validation_v2.xlsx")
TEMP_COPY = os.path.join(HERE, "_TEMP_error_injection_copy_v2.xlsx")
...
def main():
    ...
    log.append(f"\n4. Deleted temporary copy `{os.path.basename(TEMP_COPY)}`. Only the correct workbook "
               f"(`independent_dcf_validation_v2.xlsx`) is retained.\n")
